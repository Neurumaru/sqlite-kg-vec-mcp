...
#### 4. 하드코딩된 임베딩 차원 제거
- **위치**: `src/adapters/ollama/nomic_embedder.py:113, 116, 119, 151`
- **문제**: 768 차원이 하드코딩됨
- **해결**: 설정으로 외부화
- **예상 시간**: 20분
...
#### 6. 프로덕션 시뮬레이션 코드 제거
- **위치**: `src/adapters/ollama/ollama_llm_service.py`
- **문제**: 프로덕션 코드에 sleep(0.05) 시뮬레이션 지연
- **해결**: 설정 가능한 지연 또는 제거
- **예상 시간**: 15분

#### 7. 유사도 임계값 하드코딩 제거
- **위치**: `src/domain/services/knowledge_search.py:33`, `src/main.py`
- **문제**: 0.5 임계값 하드코딩
- **해결**: 설정 클래스로 외부화
- **예상 시간**: 20분
...
#### **Phase 2 (High Priority - 3시간)**
3. 타입 힌트 및 None 기본값 개선 완료 (60분)
4. 하드코딩된 값 설정화 (임베딩 차원, 가중치, 임계값) (85분)
5. 프로덕션 시뮬레이션 코드 제거 (15분)
- **목표**: 코드 품질 향상, 설정 외부화

#### **Phase 3 (Medium Priority - 7시간)**
6. 예외 처리 표준화 (120분)
7. 검증 규칙 및 타임아웃 표준화 (55분)
8. 로깅 패턴 일관성 및 설정 검증 강화 (75분)
9. 메트릭 수집 구현 (60분)
10. 트랜잭션 경계 명확화 (40분)
11. 중복 검증 로직 통합 (50분)
...
12. 테스트 최적화 및 네이밍 표준화 (70분)
13. 문서화 개선 (60분)
14. Repository 통합 및 워크플로우 캡슐화 (5-6시간)
15. 배치 처리 및 재시도 로직 (3.5시간)
