...
class TestConcurrencyIssues(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.shared_knowledge_extractor.extract.return_value = ([], [])  # (node_data_list, relationship_data_list)
    ...
    async def test_deadlock_prevention_cross_resource_access(self):
        ...
        async def process_cross_referenced_docs():
            """교차 참조 문서들을 동시 처리."""
            doc1 = Document(id=doc1_id, title="Doc1", content="References Doc2", doc_type=DocumentType.TEXT)
            doc2 = Document(id=doc2_id, title="Doc2", content="References Doc1", doc_type=DocumentType.TEXT)
    ...
    async def test_connection_pool_exhaustion_handling(self):
        ...
        if failed_results:
            connection_errors = [r for r in failed_results 
                               if isinstance(r, (ConnectionError, TimeoutError))]
    ...
    async def test_data_consistency_under_concurrent_updates(self):
        ...
        if len(consistency_repo.version_history) > 1:
            ...
            for i in range(1, len(versions)):
                ...
                self.assertEqual(version_diff, 1, 
    ...
    def test_thread_safety_shared_resources(self):
        ...
        if actual_final_value != expected_final_value:
            ...
            print(f"Race condition detected: Expected {expected_final_value}, got {actual_final_value}, loss: {data_loss}")
