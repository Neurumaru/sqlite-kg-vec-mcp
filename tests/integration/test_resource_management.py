...
class TestResourceManagement(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.knowledge_extractor.extract.return_value = ([], [])  # (node_data_list, relationship_data_list)
    ...
    async def test_memory_cleanup_after_document_processing(self):
        ...
        self.assertLess(peak_increase, 30,
                       f"Peak memory usage too high: {peak_increase:.2f}MB")
        ...
        self.assertGreater(cleanup_efficiency, 0.5,
                          "Memory cleanup efficiency should be > 50%")
    ...
    async def test_file_handle_cleanup_with_temp_files(self):
        ...
        try:
            ...
            self.assertLessEqual(final_files, initial_files + 2,
                               "File handles should be cleaned up after processing")
            
            # Verify temporary files are actually deleted
            for temp_file in temp_files_created:
                self.assertFalse(os.path.exists(temp_file),
                               f"Temporary file should be deleted: {temp_file}")
    ...
    async def test_async_task_cleanup_on_cancellation(self):
        ...
        self.assertEqual(cancelled_count, len(documents),
        ...
        self.assertEqual(len(cleanup_entries), len(documents),
                        "Cleanup should be called for each cancelled task")
        self.assertEqual(len(finally_entries), len(documents),
                        "Finally block should execute for each task")
    ...
    async def test_connection_resource_management(self):
        ...
        self.assertEqual(len(connections_opened), len(documents),
                        "One connection should be opened per document")
        self.assertEqual(len(connections_closed), len(documents),
    ...
    async def test_weak_reference_cleanup_validation(self):
        ...
        self.assertEqual(alive_objects, 0,
                        "All document objects should be garbage collected")
    ...
    async def test_resource_monitoring_during_batch_processing(self):
        ...
        self.assertGreater(len(resource_snapshots), 5,
                          "Should have multiple resource snapshots")
        
        # Check for resource leaks
        initial_snapshot = resource_snapshots[0]
        final_snapshot = resource_snapshots[-1]
        
        memory_growth = (final_snapshot['memory_rss'] - initial_snapshot['memory_rss']) / 1024 / 1024
        file_handle_growth = final_snapshot['open_files'] - initial_snapshot['open_files']
        
        self.assertLess(memory_growth, 50,
                       f"Memory growth should be reasonable: {memory_growth:.2f}MB")
        self.assertLessEqual(file_handle_growth, 2,
                           f"File handle growth should be minimal: {file_handle_growth}")
    ...
    async def test_context_manager_resource_cleanup(self):
        ...
        self.assertEqual(len(resources_acquired), 3,
                        "All resources should be acquired")
        self.assertEqual(len(resources_released), 3,
                        "All resources should be released")
        self.assertEqual(resources_acquired, resources_released,
                        "Acquired and released resources should match")
