...
class DocumentProcessor:
    ...
    def _extract_relationships(self, document: Document, nodes: List[Node]) -> List[Relationship]:
        """
        문서에서 관계를 추출합니다.
        ...
        for i, source_node in enumerate(nodes):
            for target_node in nodes[i+1:]:
                # 예시: 조직과 기술 간의 관계
                if (source_node.node_type == NodeType.ORGANIZATION and 
                    target_node.node_type == NodeType.TECHNOLOGY):
    ...
    def _link_document_to_knowledge(self, document: Document, 
                                   extraction_result: KnowledgeExtractionResult) -> None:
    ...
    def get_processing_statistics(self, documents: List[Document]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_relationships_per_document": total_relationships / processed if processed > 0 else 0
        }