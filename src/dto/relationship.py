...
class RelationshipData:
    """
    관계 데이터를 나타내는 DTO 클래스.

    Attributes:
        id: 관계의 고유 식별자
        source_node_id: 시작 노드의 식별자
        target_node_id: 끝 노드의 식별자
        relationship_type: 관계 타입 (RELATES_TO, IS_PART_OF, CONTAINS, etc.)
        properties: 관계의 속성 데이터
        embedding: 관계의 임베딩 벡터
        created_at: 관계 생성 시각
        updated_at: 관계 최종 업데이트 시각
        source_documents: 이 관계를 생성한 원본 문서들의 식별자 목록
        confidence_score: 관계 추출의 신뢰도 점수 (0.0-1.0)
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.source_node_id, str):
            raise TypeError("source_node_id는 문자열이어야 합니다")

        if not self.source_node_id.strip():
            raise ValueError("source_node_id는 공백이 아닌 문자를 포함해야 합니다")

        # target_node_id 검증
        if not isinstance(self.target_node_id, str):
            raise TypeError("target_node_id는 문자열이어야 합니다")

        if not self.target_node_id.strip():
            raise ValueError("target_node_id는 공백이 아닌 문자를 포함해야 합니다")

        # 자기 참조 관계 검증
        if self.source_node_id == self.target_node_id:
            raise ValueError(
                "source_node_id와 target_node_id는 달라야 합니다 (자기 참조 관계 불허)"
            )

        # relationship_type 검증
        if not isinstance(self.relationship_type, RelationshipType):
            raise TypeError("relationship_type은 RelationshipType enum이어야 합니다")
