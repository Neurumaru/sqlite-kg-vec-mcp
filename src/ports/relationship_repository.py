...
class RelationshipRepository(ABC):
    ...
    @abstractmethod
    async def find_by_id(self, relationship_id: RelationshipId) -> Optional[Relationship]:
        """
        ID로 관계를 찾습니다.
    ...
    @abstractmethod
    async def find_by_nodes(self, source_node_id: NodeId, target_node_id: NodeId) -> List[Relationship]:
        """
        두 노드 간의 관계들을 찾습니다.
    ...
    @abstractmethod
    async def find_by_type(self, relationship_type: RelationshipType) -> List[Relationship]:
        """
        타입으로 관계를 찾습니다.
    ...
    @abstractmethod
    async def find_by_confidence_range(self, min_confidence: float, 
                                     max_confidence: float) -> List[Relationship]:
        """
        신뢰도 범위로 관계를 찾습니다.
    ...
    @abstractmethod
    async def find_similar_relationships(self, query_vector: Vector,
                                       similarity_threshold: float = 0.5,
                                       limit: int = 10) -> List[Tuple[Relationship, float]]:
        """
        벡터 유사도로 관계를 찾습니다.
    ...
    @abstractmethod
    async def find_by_date_range(self, start_date: datetime, end_date: datetime) -> List[Relationship]:
        """
        날짜 범위로 관계를 찾습니다.
    ...
    @abstractmethod
    async def batch_update_embeddings(self, relationship_embeddings: Dict[RelationshipId, Vector],
                                    model_name: str) -> int:
        """
    ...
    @abstractmethod
    async def find_paths_between_nodes(self, source_id: NodeId, target_id: NodeId,
