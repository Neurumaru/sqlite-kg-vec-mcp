...
class LangfuseConfig(BaseSettings):
    ...
    debug: bool = Field(default=False, description="Enable debug logging for Langfuse")
...
class PrometheusConfig(BaseSettings):
    ...
    port: int = Field(default=8080, description="Prometheus metrics server port")

    host: str = Field(default="0.0.0.0", description="Prometheus metrics server host")

    path: str = Field(default="/metrics", description="Metrics endpoint path")

    namespace: str = Field(default="sqlite_kg_vec", description="Metrics namespace")

    job_name: str = Field(default="knowledge_graph", description="Job name for metrics")
...
class OpenTelemetryConfig(BaseSettings):
    ...
    service_name: str = Field(default="sqlite-kg-vec-mcp", description="Service name for tracing")
    ...
    compression: str = Field(default="gzip", description="Compression for trace export")

    timeout: float = Field(default=30.0, description="Export timeout in seconds")

    insecure: bool = Field(default=True, description="Use insecure connection for tracing")
...
class LoggingObservabilityConfig(BaseSettings):
    ...
    include_caller: bool = Field(default=False, description="Include caller information in logs")

    sanitize_sensitive_data: bool = Field(
        default=True, description="Sanitize sensitive data in logs"
    )
...
class ObservabilityConfig(BaseSettings):
    ...
    service_name: str = Field(
        default="sqlite-kg-vec-mcp", description="Service name for observability"
    )

    service_version: str = Field(default="0.2.0", description="Service version for observability")
    ...
    logging: LoggingObservabilityConfig = Field(
        default_factory=LoggingObservabilityConfig, description="Logging configuration"
    )
    ...
    trace_sampling_ratio: float = Field(default=1.0, description="Trace sampling ratio (0.0-1.0)")

    metrics_interval: float = Field(
        default=60.0, description="Metrics collection interval in seconds"
    )
