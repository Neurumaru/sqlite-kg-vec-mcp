...
class AppConfig(BaseSettings):
    ...
    def __init__(self, **kwargs):
        """Initialize app configuration with validation."""
    ...
    def _update_nested_configs(self):
        ...
        if not Path(self.database.db_path).is_absolute():
            self.database.db_path = str(Path(self.data_dir) / self.database.db_path)
        
        # Update MCP vector index directory if not set
        if self.mcp.vector_index_dir is None:
            self.mcp.vector_index_dir = str(Path(self.data_dir) / "vector_index")
    ...
    def from_env(cls, env_file: Optional[str] = None) -> "AppConfig":
        ...
        database_config = DatabaseConfig(_env_file=env_file)
        llm_config = LLMConfig(_env_file=env_file)
        mcp_config = MCPConfig(_env_file=env_file)
        observability_config = ObservabilityConfig(_env_file=env_file)
