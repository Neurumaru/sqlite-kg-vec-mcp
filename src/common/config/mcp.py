...
class MCPConfig(BaseSettings):
    ...
    # Server settings
    server_name: str = Field(default="Knowledge Graph Server", description="Name of the MCP server")
    ...
    vector_similarity: str = Field(
        default="cosine", description="Vector similarity metric (cosine, euclidean, dot)"
    )

    # Embedding settings
    embedder_type: str = Field(
        default="sentence-transformers", description="Type of text embedder to use"
    )

    embedder_model: str = Field(default="all-MiniLM-L6-v2", description="Model name for embeddings")
    ...
    search_threshold: float = Field(
        default=0.7, description="Minimum similarity threshold for search results"
    )

    # Logging
    log_level: str = Field(default="INFO", description="Logging level for MCP server")
