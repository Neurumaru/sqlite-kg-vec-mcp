...
class OllamaLLMService(LLMService):
    ...
    async def evaluate_search_results(
        ...
        for i, result in enumerate(results[:10]):  # Limit to top 10
            results_data.append({
                "index": i,
                "score": result.score,
                "snippet": getattr(result, 'snippet', '')[:100] + "..." if hasattr(result, 'snippet') else ""
            })
    ...
    async def extract_knowledge_from_text(
        ...
        result = await asyncio.to_thread(
            self.ollama_client.extract_entities_and_relationships,
            text
        )
    ...
    async def generate_search_suggestions(
        ...
        history_str = f"Recent searches: {json.dumps(search_history[-5:])}\n" if search_history else ""
