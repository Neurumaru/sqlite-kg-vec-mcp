...
class OllamaKnowledgeExtractor(KnowledgeExtractor):
    ...
    async def extract_relationships(self, text: str, entities: List[Entity]) -> List[Relationship]:
        ...
        relationships_data = await self.ollama_client.extract_relationships(text, entities)
        relationships = []
        ...
        for rel_data in relationships_data:
            try:
                ...
                logging.warning(f"Failed to create relationship from data {rel_data}: {e}")
    ...
    async def validate_extraction(self, text: str, result: ExtractionResult) -> bool:
        ...
        if result.errors:
            error_ratio = len(result.errors) / max(1, result.entities_created + result.relationships_created)
