...
async def create_sample_data(client):
    ...
    # Create person nodes
    alice = await client.request("create_node", {
        "type": "Person",
        "name": "Alice",
        "properties": {"age": 30, "occupation": "Data Scientist"}
    })
    
    bob = await client.request("create_node", {
        "type": "Person",
        "name": "Bob",
        "properties": {"age": 35, "occupation": "Software Engineer"}
    })
    
    charlie = await client.request("create_node", {
        "type": "Person",
        "name": "Charlie",
    ...
    tech_corp = await client.request("create_node", {
        "type": "Company",
        "name": "TechCorp",
        "properties": {"founded": 2010, "industry": "Technology"}
    })
    
    data_inc = await client.request("create_node", {
        "type": "Company",
        "name": "DataInc",
    ...
    await client.request("create_edge", {
...
async def query_example(client, entities):
    ...
    # Find all people
    people = await client.request("find_nodes", {
        "type": "Person",
        "limit": 10
    })
    ...
    employees = await client.request("get_neighbors", {
    ...
    # Search by text
    text_results = await client.request("search_by_text", {
        "query": "data science technology",
        "limit": 5,
        "entity_types": ["node"]
    })
